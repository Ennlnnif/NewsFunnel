...
我的日报有以下板块：
- ai_agent: AI Agent 具体产品/项目（限事实性新闻）。包括：Agent产品发布/更新/功能上线、Agent项目开源/demo、AI编程助手新版本（Cursor/Copilot/Claude Code/Manus/Hermes Agent/Harness/Evolver/OpenClaw 等功能更新）、Agent游戏化社交产品（斯坦福AI小镇/扣子养虾）。**不含**：开源 Agent SDK/框架的普通小版本更新（LangChain/CrewAI/Dify/AutoGen 等→not_relevant）、Agent基础设施科普/教程/可靠性研究（沙箱/治理/编排/权限管控技术文→not_relevant）、Agent合规风险（企业治理/合规框架→not_relevant）、Agent公司融资（→ai_business）、Agent产品的争议/吐槽/社区声讨（→opinion）。**例外（保留入 ai_agent, q=2）**：大厂官方 Agent 运行时的重大里程碑——OpenAI Agents SDK / Microsoft agent-framework / Google ADK / Anthropic Harness 的方向性/战略性更新（如模块整合、重大开源、与旗舰产品绑定），这类新闻虽是"框架"但会直接决定用户端 Agent 产品未来形态，不走 not_relevant。
...
- ai_core: AI核心技术 具体模型/系统发布（限事实性新闻）。包括：大模型发布/开源（GPT-5/Claude/Gemma新版）。**不含**：模型横评综述/行业报告（→opinion）、芯片硬件（→not_relevant）、世界模型（→ai_gaming）、TTS/语音模型（→ai_video）、具身智能/Physical AI/人形机器人/自动驾驶（→not_relevant）
...
- not_relevant: 与AI无关。也包括以下主题（日报不关注）：开源 Agent SDK/框架普通小版本更新（LangChain/CrewAI/Dify/AutoGen 等；⚠️ 例外见上方 ai_agent 定义——大厂官方 Agent 运行时的重大里程碑仍入 ai_agent）、企业级安全防护SaaS/B端Agent工具、Agent安全分析/风险解读、Agent架构科普/教程、Agent可靠性研究/基准测试、AI安全合规政策、展会相关（逛展指南/招展推广/参展广告/门票福利）、AI硬件产品、Agent基础设施科普/教程（沙箱/治理/编排/权限管控等技术文）、Agent合规风险（企业Agent治理/合规框架/风险评估）、具身智能/Physical AI/人形机器人/自动驾驶（日报不再收录此类产品/技术新闻）
...
日报不关注 → not_relevant：
❌ "LangChain v0.4.2 发布，修复几个 bug" → not_relevant（开源 Agent SDK 小版本更新，非大厂官方运行时）
❌ "CrewAI 新增一个示例 notebook" → not_relevant（开源框架普通更新）
...
❌ "一文讲透 Agent 沙箱与权限管控的 5 种模式" → not_relevant（Agent 基础设施科普/教程）
...
⚠️ 例外：大厂官方 Agent 运行时重大里程碑 → ai_agent, q=2（**不走 not_relevant**）：
✅ "Microsoft 将 Semantic Kernel 整合进 microsoft/agent-framework" → ai_agent, q=2（官方运行时模块整合，影响 Copilot 未来形态）
✅ "OpenAI Agents SDK 迎来 v1.0，与 ChatGPT Enterprise 深度绑定" → ai_agent, q=2（官方运行时与旗舰产品绑定=战略里程碑）
✅ "Google ADK 首次开源并支持 Gemini 2 原生工具调用" → ai_agent, q=2（重大开源+方向性更新）
❌ "OpenAI Agents SDK 新增一个小参数" → not_relevant（普通小版本更新，非里程碑）
...
- ai_core: 通用大模型、训练推理、架构创新。不含世界模型（→ai_gaming）、TTS（→ai_video）、具身智能/Physical AI/人形机器人/自动驾驶（→不捞回，日报不关注）
...
❌ 不该捞回: "Gemini Robotics-ER 1.6: Powering real-world robotics tasks" → 具身智能/机器人产品，日报不关注
...
"""
    ...
    def load_results(self, json_path: str | Path):
        ...
        # ── C2: 宽容型 schema 校验，自动修正类型错误 ──
        fix_count = 0
        for section in ("classify", "rescue"):
            for entry in raw.get(section, []):
                # relevant: 强制转 bool
                if "relevant" in entry and not isinstance(entry["relevant"], bool):
                    orig = entry["relevant"]
                    entry["relevant"] = str(orig).lower() in ("true", "1", "yes")
                    logger.warning(
                        f"[C2] {section}[{entry.get('id','')}].relevant "
                        f"类型修正: {orig!r} → {entry['relevant']}"
                    )
                    fix_count += 1
                # quality: 强制转 int
                if "quality" in entry and not isinstance(entry["quality"], int):
                    orig = entry["quality"]
                    try:
                        entry["quality"] = int(orig)
                    except (ValueError, TypeError):
                        entry["quality"] = 0
                    logger.warning(
                        f"[C2] {section}[{entry.get('id','')}].quality "
                        f"类型修正: {orig!r} → {entry['quality']}"
                    )
                    fix_count += 1
        if fix_count:
            logger.warning(f"[C2] schema 校验共修正 {fix_count} 处字段类型")
    ...
    # ── 代码层预过滤（2026-04-17 新增）──
    # 明显不相关的文章在代码层直接踢掉，不进入 LLM classify 候选
    # 目的：减少 LLM 需要判断的文章数，节省 token

    # 第一级：游戏关键词 + 无 AI 信号词 → 踢掉
    _GAME_KEYWORDS = [
        "蛋仔", "原神", "崩坏", "逆水寒", "星穹铁道", "星露谷", "卡普空", "R星",
        "保底", "命途", "赠礼", "时装", "皮肤", "版本概念预告",
        "畅销总榜", "人事", "评测",
    ]
    _AI_SIGNAL_WORDS = [
        "AI", "ai", "模型", "NPC", "Agent", "agent", "UGC", "ugc",
        "生成", "大模型", "LLM", "llm", "世界模型", "AIGC", "aigc",
    ]
    # 第二级：直接踢掉（任何标题含即踢）
    _ALWAYS_REJECT_KEYWORDS = [
        "招聘", "申报", "征集", "报名", "评选启动",
        "展会", "参展", "逛展", "门票", "免费票",
        "去世", "讣告",
    ]

    def _pre_filter(self, articles: list[dict]) -> tuple[list[dict], int]:
        """代码层预过滤：明显不相关的文章直接踢掉

        两级规则：
        1. 游戏关键词 + 不含 AI 信号词 → 踢掉
        2. 招聘/展会/讣告等 → 直接踢掉
        ...
        for art in articles:
            ...
            # 第二级：直接踢掉
            if any(kw in title for kw in self._ALWAYS_REJECT_KEYWORDS):
                art["_relevance_tags"] = []
                art["_relevance_priority"] = "none"
                art["filter_reason"] = "pre_filter_reject"
                removed += 1
                logger.info(f"  预过滤踢掉(直接): {title[:40]}")
                continue

            # 第一级：游戏关键词 + 无 AI 信号词
            has_game_kw = any(kw in title for kw in self._GAME_KEYWORDS)
            if has_game_kw:
                has_ai_signal = any(kw in title for kw in self._AI_SIGNAL_WORDS)
                if not has_ai_signal:
                    art["_relevance_tags"] = []
                    art["_relevance_priority"] = "none"
                    art["filter_reason"] = "pre_filter_game_no_ai"
                    removed += 1
                    logger.info(f"  预过滤踢掉(游戏无AI): {title[:40]}")
                    continue
    ...
    def run(self, keyword_passed: list[dict],
        ...
        """执行完整 LLM 分类+捞漏流程"""
        # ── 代码层预过滤：明显不相关的文章直接踢掉，不进入 LLM 候选 ──
        keyword_passed, pre_filtered_count = self._pre_filter(keyword_passed)
        if pre_filtered_count:
            console.print(f"  🗑️ 代码层预过滤: 踢掉 {pre_filtered_count} 篇明显不相关")
...
def _generate_llm_results_template(passed: list[dict], today_dir: Path):
    ...
    for key in ordered_keys:
        ...
        template[key] = {
            "articles": articles,
            "insight": "__TODO__" if len(articles) >= 3 else "",
        }
    ...
    for key in section_map:
        if key not in template:
            template[key] = {
                ...
                "insight": "__TODO__" if len(section_map[key]) >= 3 else "",
            }
