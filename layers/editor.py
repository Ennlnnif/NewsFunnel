...
**⚠️ 全文阅读规则（按 channel 区分，必须严格遵守）**：

- **channel=twitter**：推文全文已在文章对象的 `summary` 字段中（Layer 1 通过 xreach 已采集完整推文文本）。
  **直接基于 summary 生成，禁止 web_fetch `x.com` / `twitter.com` 链接（有登录墙，注定失败）**。
  仅当推文引用了外部 URL（论文/产品页/文章）且核心信息依赖该外链，才 web_fetch 那个外部链接。

- **channel=wechat**：**必须通过 web_fetch 打开原文 `mp.weixin.qq.com/s/...` URL 读全文**。
  Layer 1 的 summary 仅作为初筛依据，不作为生成 summary 的事实来源。
  若 web_fetch 返回"请在微信客户端打开"或 404，fallback 使用 summary 字段，并在 summary 末尾标注"（基于摘要生成）"。

- **channel=rss**：**必须通过 web_fetch 打开原文 URL 读全文**。
  web_fetch 失败时（429/403/timeout）按以下顺序兜底：
  ① 查看文章对象是否有 `content` 字段（RSS `content:encoded`），有则使用；
  ② 否则 fallback 到 `summary` 字段，并在 summary 末尾标注"（基于摘要生成）"。

- **channel=exa**：`summary` 字段已含 Exa 抓取的正文片段（~500 字），通常可直接使用；如需更深入可 web_fetch 补充。

- **channel=github**：项目 README/description 已在 `summary`/`extra` 中，无需 web_fetch。

通用规则：禁止仅凭标题生成内容——标题常有夸张/省略。

任务：
1. 为每篇文章生成：
   - summary: 一句话核心摘要（15-80字中文，硬约束），格式要求见下方
   - keywords: 3-4 个信息补充关键词，要求见下方
2. 为整个板块生成：
   - insight: 板块洞察（≤80字），要求见下方。**如果板块文章少于 3 篇，可以省略 insight（填空字符串）**
...
3. **15-80字**——像新闻副标题一样精炼，只保留最核心的一个事件。下限 15 字的用意：避免输出孤立的短语片段
...
8. **技术方案、功能细节、架构组件、实验数据不要放在 summary 里**——这些信息放 keywords（除非是文章的核心观点）
9. **summary 和 keywords 不允许信息重复**——summary 中已出现的产品名/厂商名/事件描述不要在 keywords 中重复

好的示例（符合 15-80 字，产品名明确，无多余数字）：
- ✅ "OpenAI 重构 Codex，加码桌面控制与企业插件生态对标 Claude Code。"（30 字）
- ✅ "蚂蚁 CodeFuse 提出 NES：连按 Tab 完成跨文件重构，不用写 Prompt。"（32 字）
- ✅ "Anthropic 发 Claude Design 直捣设计软件腹地，Figma/Adobe 股价应声下跌。"（36 字）

差的示例：
- ❌ "企业版 Agent 部署方案上线，将推理与执行解耦"（缺产品名）
- ❌ "OpenAI 同日流失 Sora 创始人 Bill Peebles、AI for Science VP Kevin Weil、企业应用 CTO Narayanan 三位高管；股东讨论由 Bret Taylor 接替 Altman 出任 IPO 后 CEO，估值 8500 亿。"（超约束、人名和数字堆砌）
...
**⚠️ 数据来源规则（必须严格遵守）**：
- 推文完整文本已在文章对象的 `summary` 字段中（Layer 1 通过 xreach 已采集完整推文文本）。
- **直接基于 summary 生成，禁止 web_fetch `x.com` / `twitter.com` 链接**——这些链接有登录墙，注定失败，撞墙后被迫标注"基于摘要生成"纯属浪费时间。
- 仅当推文引用了外部 URL（论文/产品页/文章）且核心信息依赖该外链，才去 web_fetch 那个外部链接。
- 跨语种：英文推文必须翻译成中文 summary，保留英文产品名/人名/术语不翻译。

任务：
1. 为每条推文生成：
   - summary: 一句话核心摘要（15-80字中文，硬约束），格式：[谁] + [说了什么/做了什么] + [关键信息/影响]
   - keywords: 3-4 个信息补充关键词（技术细节、功能亮点、数字放这里，不重复 summary 中的人名/产品名）
2. 为整个 Twitter 板块生成：
   - insight: 板块洞察（≤80字，回答why/how，揭示因果与影响，可选务实启示）。**如果推文少于 3 条，可以省略 insight（填空字符串）**

## summary 写作规范
1. **聚焦 what + so what**——这条推文的核心信息 + 为什么值得关注
2. **必须出现具体人名/产品名**——禁止"研究人员""新工具"等模糊表述
3. **15-80字**——像新闻副标题一样精炼，下限 15 字避免输出碎短片段
...
**⚠️ 阅读要求（按数据可用性决定是否 web_fetch）**：
- 文章对象的 `summary` / `extra` 中通常已包含 Layer 1 抓取的项目描述（GitHub Trending RSS 已带 description）。
- 若 summary 足以说清项目定位（做什么 + 核心亮点），**直接基于 summary 生成，无需 web_fetch**。
- 若 summary 过短或信息不足，才 web_fetch 项目首页读 README.md（README 通常足够，无需读源码）。
- 如果链接无法访问且 summary 也不足，在 summary 末尾标注"（基于摘要生成）"。

任务：
1. 为每个项目生成：
   - summary: 一句话核心摘要（15-80字中文，硬约束），格式：[项目名] + [做什么/解决什么问题] + [关键亮点/影响]
   - keywords: 3-4 个信息补充关键词（技术栈、架构方案、适用场景放这里，不重复 summary 中的项目名）
2. 为整个 GitHub 板块生成：
   - insight: 板块洞察（≤80字，回答why/how，揭示开源趋势与影响，可选务实启示）。**如果项目少于 3 个，可以省略 insight（填空字符串）**

## summary 写作规范
1. **必须以项目名开头**——如 "markitdown: 微软开源的多格式转Markdown工具"
2. **聚焦 what + so what**——说清楚项目做什么、解决什么问题、为什么值得关注
3. **15-80字**——像新闻副标题一样精炼，下限 15 字避免仅有项目名
...
- **channel=twitter**：推文全文已在 `summary` 字段（Layer 1 已抓）。直接基于 summary 生成，**禁止 web_fetch `x.com` / `twitter.com`**（登录墙）。仅当引用了外部论文/文章链接，才 web_fetch 那个外部链接。

- **channel=wechat**：**必须通过 web_fetch 打开 `mp.weixin.qq.com/s/...` 读全文**，观点类文章的论点往往在正文中展开。失败时 fallback 到 summary，并在末尾标注"（基于摘要生成）"。

- **channel=rss**：**必须通过 web_fetch 打开原文 URL 读全文**。失败时先看 `content` 字段，再 fallback 到 summary（末尾标注"（基于摘要生成）"）。

- **channel=exa**：`summary` 字段已含正文片段，可直接用；如需补充论据再 web_fetch。

- **channel=github**：观点板块一般不涉及。

通用规则：观点类文章的核心论点往往在正文展开，标题常无法体现完整逻辑。

任务：
1. 为每篇文章生成：
   - summary: 一句话核心摘要（15-80字中文，硬约束），格式：[谁] + [核心观点/发现] + [为什么重要]
   - keywords: 3-4 个信息补充关键词（论据细节、数据支撑、适用场景放这里，不重复 summary 中的人名/公司名）
2. 为整个观点板块生成：
   - insight: 板块洞察（≤80字，回答why/how，揭示观点背后的共性趋势，可选务实启示）。**如果文章少于 3 篇，可以省略 insight（填空字符串）**

## summary 写作规范
1. **聚焦 what + so what**——提炼出"他认为什么/发现了什么"+"为什么这个观点值得关注"
2. **必须出现具体人名/公司名/产品名**——禁止"业内人士""专家"等模糊表述
3. **15-80字**——像新闻副标题一样精炼，下限 15 字避免表达不清
4. **不要堆砌数字**——最多保留 1 个最关键的数字
...
def _render_main_article(art: dict, llm_data: dict) -> str:
    ...
    _PLACEHOLDERS = ("__TODO__", "__MUST_FETCH__")
    if summary and not any(p in summary for p in _PLACEHOLDERS):
        headline = summary
    else:
        if summary:
            import logging as _logging
            _logging.getLogger(__name__).warning(
                f"[C1] 占位符未替换，fallback 到 title: {title!r}"
            )
        headline = title
